...
### **禁止在 zero-peak / FelisCatus 任意分支网页上点击 Sync fork 按钮**
Sync fork 仅绑定原始上游 FelisCatus/SwitchyOmega，一旦执行会覆盖 zero-peak 第三方定制代码，引发大规模代码丢失与冲突。
本仓库的代码分支并非父仓库的直接衍生版，使用网页同步会破坏分支历史。

所有上游代码更新，仅允许通过本地 Git 命令行同步，禁止网页一键同步 Fork。
更新请使用命令行：

```bash
# 同步 FelisCatus
git checkout FelisCatus
git -c http.proxy=http://127.0.0.1:10808 -c https.proxy=http://127.0.0.1:10808 pull upstream master
git -c http.proxy=http://127.0.0.1:10808 -c https.proxy=http://127.0.0.1:10808 push origin FelisCatus
```
